...
def save_loss_plot(losses, grad_norms, save_dir):
	fig, (ax1, ax2) = plt.subplots(2, 1, figsize=(8, 6))

	window = max(1, len(losses) // 200)
	kernel = np.ones(window) / window

	ax1.plot(np.convolve(losses, kernel, mode="valid"))
	ax1.set_ylabel("L_simple (MSE)")
	ax1.set_title("Training loss")

	ax2.plot(np.convolve(grad_norms, kernel, mode="valid"))
...
def train(args):
	...
	loss_window = []  # rolling window for smooth loss
	grad_norms = []
	...
	while step < args.steps:
		...
		pbar.set_postfix(
			{
				"loss": f"{loss.item():.4f}",
				"loss_avg": f"{np.mean(loss_window):.4f}",
		...
		if step % 500 == 0 or step == 1:
			tqdm.write(
				...
				f"loss={np.mean(loss_window):.5f}  "
				f"grad_norm={np.mean(grad_norms[-500:]):.4f}  "
				f"lr={optimizer.param_groups[0]['lr']:.1e}"
			)

		# ── Periodic sampling ─────────────────────────────────────────────
		if step % args.sample_every == 0 or step == args.steps:
			tqdm.write(f"[step {step}] Generating samples...")
			...
			tqdm.write(f"[step {step}] Samples saved.")
	...
	# ── Final plots ────────────────────────────────────────────────────────
	save_loss_plot(losses, grad_norms, args.save_dir)
	# Save loss values as numpy for later analysis
	np.save(os.path.join(args.save_dir, "losses.npy"), np.array(losses))
	np.save(os.path.join(args.save_dir, "grad_norms.npy"), np.array(grad_norms))
